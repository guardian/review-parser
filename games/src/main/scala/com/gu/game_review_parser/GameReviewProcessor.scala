...
object GameReviewProcessor {
  def processSearchQuery(pages: Seq[Int], capiClient: GuardianContentClient, query: SearchQuery): Seq[ParsedGameReview] = {
    pages.flatMap { page =>
      ...
      Try(Await.result(capiClient.getResponse(query.page(page)), 5.seconds)) match {
        case Success(response) => response.results.flatMap(StandardParser.parseContent)
        case Failure(e) =>
  ...
  def processItemQuery(capiClient: GuardianContentClient, query: ItemQuery): Option[ParsedGameReview] = {
    Try(Await.result(capiClient.getResponse(query), 5.seconds)) match {
      case Success(response) => response.content.flatMap(StandardParser.parseContent)
      case Failure(e) =>
